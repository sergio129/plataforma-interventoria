...
export default function UsuariosPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (res.ok && j.success) {
        // backends may return wrapped data
  ...
  function openEdit(u: Usuario) { setEditing(u); setShowForm(true); }
  ...
  function askDelete(id?: string) { if (!id) return; setToDelete(id); setConfirmOpen(true); }
...
function UserForm({ initial, onSave, onCancel, roles }: { initial?: Usuario|null; roles: Rol[]; onSave: (u:any)=>Promise<void>; onCancel: ()=>void }) {
  ...
  return (
    <form onSubmit={submit}>
      ...
      {/* Tipo se asigna vía Roles seleccionados; no mostrar campo aquí */}
